...
async function generateAnalysisReport(ticket) {
  if (!ticket || !ticket.numero) throw new Error('Ticket invalide');
  ensureDir();
  const fileName = `${ticket.numero}.pdf`;
  ...
  // ----- Titre & ref -----
  doc.fillColor('#1a1a1a').fontSize(16).font('Helvetica-Bold').text(`Rapport d'analyse n° ${ticket.numero}`);
  doc.fontSize(10).font('Helvetica').fillColor('#475569')
    .text(`Date d'analyse : ${fmtDate(ticket.updatedAt || new Date())}    ·    Ouvert le : ${fmtDate(ticket.sla && ticket.sla.dateOuverture)}`);
  doc.moveDown(1);

  // ----- Pièce -----
  doc.fillColor('#1a1a1a').fontSize(12).font('Helvetica-Bold').text('1. Identification de la pièce');
  ...
  // ----- Véhicule -----
  doc.fontSize(12).font('Helvetica-Bold').text('2. Véhicule');
  ...
  // ----- Symptômes déclarés -----
  doc.fontSize(12).font('Helvetica-Bold').text('3. Symptômes déclarés par le client');
  doc.font('Helvetica').fontSize(10);
  ...
  // ----- Procédure de test -----
  doc.fontSize(12).font('Helvetica-Bold').text('4. Procédure de test sur banc');
  doc.font('Helvetica').fontSize(10).fillColor('#1a1a1a');
  ...
  // ----- Résultats -----
  doc.fontSize(12).font('Helvetica-Bold').text('5. Résultats du banc');
  doc.font('Helvetica').fontSize(10);
  ...
  if (ticket.diagnostic && ticket.diagnostic.scoreRisque != null) {
    doc.text(`Score de risque interne : ${ticket.diagnostic.scoreRisque} / 100`);
  }

  // Photos banc (si URLs http)
  const photos = (a.photosBanc || []).slice(0, 4);
  if (photos.length) {
    doc.moveDown(0.4);
  ...
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#1a1a1a').text('6. Conclusion');
